...
class TLDRFormatter(Formatter):
    ...
    def _is_function_definition(self, tokens, start_idx):
        ...
        # Get function name - prioritize Name.Function but fall back to others
        ttype, value = tokens[i]
        if ttype == Name.Function:
            # High confidence this is a function definition
            function_name = value
            found_name = True
            logging.info(f"Found function definition: {function_name} at index {i} ttype={ttype}")
            i += 1
        # elif ttype in (Name, Name.Other) and found_def:
        #     # Only accept other name types if we already found a definition keyword
        #     function_name = value
        #     found_name = True
        #     # logging.info(f"Maybe Found function definition: {function_name} at index {i} ttype={ttype}")
        #     i += 1
    ...
    def format_unencoded(self, tokensource, outfile):
        ...
        if self.highlight_functions:
            i = 0
            while i < len(tokens):
                # if token == function definition keyword then log it
                if tokens[i][0] in (Name.Function):
                    logging.debug(f"Found potential function definition at index {i}: {tokens[i][1]}")
        ...
        if function_signatures:
            ...
            for func in function_signatures:
                ...
                if self.linenos:
                    ...
                    outfile.write(f'- **{func["name"]}**{params_display}\n')
        ...
        if self.options.get('full'):
            ...
            outfile.write('SKIPPED OUTPUTTING FILE CONTENTS - FULL DOCUMENT MODE IS OFF\n')
